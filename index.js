...
// --- TRANSLATIONS ---
// Fix: Updated `imageSavedToDownloads` to refer to "Documents" folder.
const translations = {
...
const PaletteButton = ({ active, onClick, children }) => (
    React.createElement("button", { onClick, className: `px-4 py-2 text-sm font-semibold rounded-md transition ${active ? 'bg-sky-600 text-white' : 'bg-slate-700 hover:bg-slate-600 text-slate-300'}` }, children)
);

const ColorSwatch = ({ color, isSelected, onClick }) => (
    React.createElement("button", { onClick, className: `w-12 h-12 rounded-full transition-transform duration-200 transform hover:scale-110 shadow-md border-4 ${isSelected ? 'border-sky-400 scale-110' : 'border-transparent'}`, style: { backgroundColor: color }, "aria-label": `Select color ${color}` })
);
        ...
        React.createElement("div", null,
            ...
            React.createElement("div", { className: "mb-6" },
                ...
                React.createElement("div", { className: "flex space-x-2" },
                    React.createElement(PaletteButton, { active: paletteSize === 2, onClick: () => onPaletteSizeChange(2) }, `2 ${t('colors')}`),
                    React.createElement(PaletteButton, { active: paletteSize === 4, onClick: () => onPaletteSizeChange(4) }, `4 ${t('colors')}`)
                )
            ),
            React.createElement("div", { className: "mb-6" },
                ...
                React.createElement("div", { className: "flex flex-wrap gap-4" },
                    currentPalette.map((color, index) => (
                        React.createElement("div", { key: index, className: "flex flex-col items-center gap-2 p-2 rounded-lg bg-slate-900/50" },
                            React.createElement(ColorSwatch, { color, isSelected: selectedColor === color, onClick: () => onSelectColor(color) }),
...
const App = () => {
    ...
    const handleCreateImage = useCallback(async () => {
        ...
        try {
            const dataUrl = await toPng(resultsRef.current, { cacheBust: true, backgroundColor: '#1e293b' });
    ...
    const { unassigned, assigned } = useMemo(() => ({
        unassigned: people.filter(p => !p.color),
        assigned: currentPalette.map(color => ({
            color,
    ...
    const header = React.createElement("header", { className: "w-full max-w-7xl mx-auto flex justify-between items-center mb-8" },
    ...
    const mainContent = step === 'input'
        ? React.createElement(NameInputStep, { rawNames, setRawNames, onProcess: handleProcessNames })
        : React.createElement("div", { className: "flex flex-col xl:flex-row gap-8" },
            React.createElement("div", { className: "w-full xl:w-1/3" },
                React.createElement(AssignmentStep, { paletteSize, onPaletteSizeChange: handlePaletteSizeChange, currentPalette, onPaletteColorChange: handlePaletteColorChange, selectedColor, onSelectColor: setSelectedColor, assignedCount, totalCount: people.length })
            ),
                    ...
                    React.createElement("ul", { className: "space-y-2" },
                        unassigned.map(person => (
                            React.createElement("li", { key: person.id },
                                React.createElement("button", { onClick: () => handleAssignColor(person.id), className: "w-full text-left p-3 rounded-md bg-slate-700 hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-sky-500 transition" }, person.name)
                            )
                        ))
                    )
                ),
    ...
    const footer = step === 'assign' && React.createElement("footer", { className: "w-full max-w-7xl mx-auto mt-8 flex flex-col sm:flex-row justify-center items-center gap-4" },
    ...
    return React.createElement("div", { className: "min-h-screen flex flex-col p-4 sm:p-8" },
        header,
        React.createElement("main", { className: "flex-grow w-full max-w-7xl mx-auto" }, mainContent),
        footer
    );
};
